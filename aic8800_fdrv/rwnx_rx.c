 ...
 ******************************************************************************
 */
#include <linux/version.h>     // <-- ОБЯЗАТЕЛЬНО
#include <linux/kernel.h>      // <-- желательно
#include <linux/dma-mapping.h>
#include <linux/ieee80211.h>
#include <linux/etherdevice.h>
#include <linux/timer.h>        // уже есть
...
check_len_update:
    ...
    if (status & RX_STAT_SPURIOUS) {
        ...
        if (rwnx_vif) {
            cfg80211_rx_spurious_frame(rwnx_vif->ndev, hdr->addr2, GFP_ATOMIC, GFP_ATOMIC);
        }
        goto end;
    }
        ...
        if (hw_rxhdr->flags_is_80211_mpdu) {
            ...
            if (hw_rxhdr->flags_sta_idx != RWNX_INVALID_STA) {
                ...
                if (hw_rxhdr->flags_is_4addr && !rwnx_vif->use_4addr) {
                    cfg80211_rx_unexpected_4addr_frame(rwnx_vif->ndev, sta->mac_addr, GFP_ATOMIC, GFP_ATOMIC);
                }
            }
